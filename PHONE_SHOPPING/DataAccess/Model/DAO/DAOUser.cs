...
{
    public class DAOUser : PHONE_SHOPPINGContext
    {
        public async Task<User?> getUser(Guid UserID)
        {
            return await Users.Include(u => u.Role).SingleOrDefaultAsync(u => u.UserId == UserID);
        }
        ...
        {
            return await Users.SingleOrDefaultAsync(u => u.Email == email.Trim());
        }
        ...
        {
            return await Users.AnyAsync(u => u.Email == email.Trim() && u.UserId != UserID);
        }
        public async Task<List<string>> getEmailAdmins()
        {
            return await Users.Where(u => u.RoleId == RoleConst.ROLE_ADMIN).Select(u => u.Email).ToListAsync();
        }
